...
const GalleryUploadForm = () => {
  ...
  const [getUploadGalleryImageUrl] = useGetUploadGalleryImageUrlMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setUploading(true);
    try {
      const { name: fileName, type: fileType } = galleryImage;

      const { uploadUrl, imageUrl } = await getUploadGalleryImageUrl({
        fileName,
        fileType,
      }).unwrap();
      ...
      const res = await saveGalleryImage({ imageUrl }).unwrap();
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 bg-white shadow-lg p-6 rounded-md">
      <h2 className="text-2xl font-semibold mb-4 text-blue-900">Upload Gallery Image</h2>
      ...
      <h3 className="text-xl font-semibold mb-4 text-blue-800">Gallery</h3>
      {isGalleryLoading ? (
        <p>Loading images...</p>
      ) : (
